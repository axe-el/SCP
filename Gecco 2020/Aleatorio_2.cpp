...
void iniciacion_random2(vector<int> &s1,vector<int> &cobertura_s1,vector < vector<int> > &filas,vector < vector<int> > &columnas);
...
int main(int argc, char *argv[]){
	...
	if(argc==4){
		...
		int costo_s;
		//Vector solucion
		vector<int> s1(columnas.size(),1);
		//Vector cobertura filas
		vector<int> cobertura_s1(filas.size(),0);
		Cobertura(s1,cobertura_s1,columnas);		
		
		//Solucion random y calculamos su costo
		iniciacion_random2(s1,cobertura_s1,filas,columnas);		
		costo_s = Costo_1(s1);	
...
//Solucion aleatoria vamos quitando columnas
void iniciacion_random2(vector<int> &s1,vector<int> &cobertura_s1,vector < vector<int> > &filas,vector < vector<int> > &columnas){

	//Orden en el que visitar el #filas
	vector<int> orden(s1.size(),0);
		...
		if( s1[ columna ] == 1){
			...
			for(int j = 0 ; j< columnas[columna].size() ; j++) {	
				...
				if( cobertura_s1[ columnas[columna][j] ] <= 0 ){
					//Volvemos a 1 esa posicion(Agregamos columna a la solucion)
					s1[ columna ] = 1;
					//Sumamos 1 a cada entrada de las que quitamos(No a todas)del vector cobertura que cubria esa columna 
					for(int k = 0 ; k<= j ; k++) {
